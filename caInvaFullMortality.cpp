...
//
//
//  VERSION 20000601 FULL MORTALITY de Juveniles debido a Adultos!!!!!!!!!!!!!!!!1 
//
//			Primero cuenta la cantidad de adultos en el entorno y luego realiza la
//				mortalidad de Juveniles debido a adultos
//
...
void CAInva::Growth(int x,int y)
{
	...
	if(ActualSp>0 )
	{
		...
		if( isAdult )
		{
			dd = Sp[actSp].MeanDispersalDistance; // Distancia Media en todos los casos
...
//            superficie = M_PI * kn * kn;
			if(kn!=0)
			{
// Cuenta la cantidad de Adultos en el entorno
//				
...
// Ejecuta mortalidad de Juveniles debida a adultos
//				
				for(dx=x-kn; dx <= x+kn; dx++ )
				{
					xdif=dx-x;
